...
const RIDVAN_182_BE =
  'https://universalhouseofjustice.bahai.org/ridvan-messages/20250420_001';
...
const Home = ({navigation, homeOverview, route}) => {
  ...
  const isLoading =
    userActivities === null || userEvents === null || userPosts === null;
  ...
  const eventWithoutHost = useMemo(() => {
    ...
    return (
      upcoming.find(
        e => !e.hosts || (Array.isArray(e.hosts) && e.hosts.length === 0),
      ) || null
    );
  }, [homeOverview?.events]);
  ...
  const activeTabPosition = useRef(
    new Animated.Value(FEATURE_TABS.indexOf(activeTab)),
  ).current;
  ...
  const clampedScroll = useMemo(
    () =>
      Animated.diffClamp(
        scrollY,
        -BANNER_PULL_EXPANSION,
        IMAGE_BANNER_HEIGHT - MIN_BANNER_HEIGHT,
      ),
    [scrollY],
  );
    ...
    () =>
      isBannerLocked
        ...
        : clampedScroll.interpolate({
            ...
            outputRange: [
              IMAGE_BANNER_HEIGHT + BANNER_PULL_EXPANSION,
              IMAGE_BANNER_HEIGHT,
              MIN_BANNER_HEIGHT,
            ],
            extrapolate: 'clamp',
  ...
  const recentPosts = useMemo(() => {
    const overviewPosts = Array.isArray(homeOverview?.posts)
      ? homeOverview.posts
      : [];
  ...
  const iosVersion =
    Platform.OS === 'ios'
      ? typeof Platform.Version === 'string'
        ...
        : Platform.Version
      : 0;
  const useIconGlassButtons =
    Platform.OS === 'ios' && Number.isFinite(iosVersion) && iosVersion >= 26;
  ...
  return (
    <View style={{flex: 1}}>
      ...
      <View style={styles.container}>
        ...
        <Animated.View
          style={[styles.bannerShadow, {height: bannerHeight}]}
          pointerEvents="box-none">
          <ImageBanner
            ...
            <View
              ...
              {(() => {
                ...
                const gregorianDate = nowDate.toLocaleDateString(undefined, {
                  month: 'short',
                  day: 'numeric',
                  year: 'numeric',
                });
            ...
            <Animated.View style={{opacity: contentOpacity}}>
              ...
              <Animated.View style={{transform: [{translateX: slideAnim}]}}>
                ...
                {activeTab === 'Events' &&
                  ...
                  (() => {
                    ...
                    const eventDateTime = eventDate.toLocaleString(undefined, {
                      weekday: 'short',
                      month: 'short',
                      day: 'numeric',
                      hour: 'numeric',
                ...
                {activeTab === 'Assembly' &&
                  (() => {
                    if (!Array.isArray(homeOverview?.events)) return null;
                    const now = new Date();
                    const lsaEvents = homeOverview.events.filter(
                      e => e.title === 'Local Spiritual Assembly Meeting',
                    );
                    let nextLsaEvent = null;
                    if (lsaEvents.length > 0) {
                      const futureEvents = lsaEvents.filter(
                      ...
                      if (futureEvents.length > 0) {
                        ...
                        const pastEvents = lsaEvents.filter(
                          e => e.startTime && new Date(e.startTime) < now,
                        );
                        ...
                        <View style={styles.dualGrid}>
                          ...
                          <View style={styles.smallTilesColumn}>
                            ...
                            <SquareTile
                              ...
                              style={styles.smallTileLast}
                              onPress={() =>
                                navigation.navigate('RequestAgendaItem')
                              }
                            />
                          </View>
                        </View>
                    ...
                    return (
                      <View style={styles.dualGrid}>
                        <RectangularTile
                          ...
                          bgImgColour="blue"
                          imageSource={
                            localImages[nextLsaEvent.imageUrl] || {
                              uri: nextLsaEvent.imageUrl,
                            }
                          }
                ...
                {activeTab === 'Activities' &&
                  Array.isArray(homeOverview?.activities) &&
                  homeOverview?.activities.length > 0 &&
                  (() => {
                    const now = new Date();
                    ...
                    const activityToFacilitate =
                      upcoming.find(a => {
                        const currentCount = Array.isArray(a.facilitators)
                          ? a.facilitators.length
                          : 0;
                        return (
                          typeof a.facilitatorLimit === 'number' &&
                          currentCount < a.facilitatorLimit
                        );
                      }) || null;
                    ...
                    const timeLabel =
                      formatGroupTime(nextAct?.groupDetails?.time) ||
                    ...
                    return (
                      <View style={styles.dualGrid}>
                        <View style={styles.smallTilesColumn}>
                          {secondPost && (
                            <SquareTile
                              ...
                              onPress={() =>
                                navigation.navigate('Feed', {post: secondPost})
                              }
                              style={styles.smallTileGap}
                            />
                          )}
                        ...
                        <RectangularTile
                          ...
                          imageSource={{
                            uri: firstPost.mediaThumbnails[0] || null,
                          }}
                          onPress={() =>
                            navigation.navigate('Feed', {post: firstPost})
                          }
                          style={styles.largeTile}
                        />
                      </View>
              ...
              {recentPosts.length > 0 && (
                <View style={styles.recentPostsSection}>
                  <Text style={[styles.heading, styles.recentHeading]}>
                    Recent Posts
                  </Text>
                  <FlatList
                    data={recentPosts}
                    keyExtractor={(item, index) =>
                      item?._id?.toString?.() || `recent-${index}`
                    }
                    horizontal
                    ...
                    ItemSeparatorComponent={() => (
                      <View style={{width: RECENT_CARD_GAP}} />
                    )}
                    renderItem={({item}) => {
